...
describe('ServiceEditorView', () => {
  ...
  it('preserves loaded service icon type and previews the primary favicon', async () => {
    ...
    await vi.waitFor(() => {
      expect(wrapper.get('select[name="service-icon-type"]').element).toHaveProperty(
        'value',
        'favicon',
      )
    })
    ...
    expect(wrapper.find('img[alt="Grafana 图标"]').attributes('src')).toBe(
      'https://grafana.example.test/favicon.ico',
    )
